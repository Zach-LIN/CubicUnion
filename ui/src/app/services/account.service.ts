...
export class AccountService {
  ...
  public getAccount(accNumber: string) {
    return this.httpClient.get(`${environment[10001].api_url}/account/${accNumber}`);
  }
  
 public getTransactions(accNumber: string) {
    return this.httpClient.get(`${environment[10001].api_url}/transactions/${accNumber}`);
  }
  ...
  public postTransfer(fromAccNumber: string, toBankID: string, toAccNumber: string, amount: string){
    ...
    return this.httpClient.post(`${environment[10001].api_url}/transfer`, transfer);
  }
}
